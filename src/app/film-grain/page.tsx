...
import { ChevronLeft, Wind, Download, RefreshCcw, Loader2 } from 'lucide-react';
...
export default function FilmGrainPage() {
  ...
  const processImage = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          saveAs(blob, `grain_${originalImage.file.name}`);
          toast({ title: "Selesai", description: "Tekstur film grain telah ditambahkan." });
        }
    ...
    <div className="container mx-auto p-4 sm:p-8 max-w-6xl space-y-10">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-black uppercase tracking-tight">Film Grain</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Vintage Cinematic Texture</p>
        </div>
      </div>
            ...
            <Card className="rounded-3xl border-none shadow-xl overflow-hidden">
              <CardHeader className="bg-muted/50 border-b py-4">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <Wind className="w-4 h-4 text-accent" /> Texture Level
                </CardTitle>
              </CardHeader>
