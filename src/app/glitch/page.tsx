...
export default function GlitchPage() {
  ...
  const processImage = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          saveAs(blob, `glitch_${originalImage.file.name}`);
          toast({ title: "Selesai", description: "Efek digital glitch telah diterapkan." });
        }
    ...
    <div className="container mx-auto p-4 sm:p-8 max-w-6xl space-y-10">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-black uppercase tracking-tight">Glitch Maker</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Digital Distortion Art</p>
        </div>
      </div>
            ...
            <Card className="rounded-3xl border-none shadow-xl overflow-hidden">
              <CardHeader className="bg-muted/50 border-b py-4">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <IterationCcw className="w-4 h-4 text-accent" /> Distortion Level
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-8">
                <div className="space-y-4">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground">Chaos: {intensity[0]}</Label>
                  <Slider value={intensity} onValueChange={setIntensity} min={5} max={100} step={1} />
                </div>
