...
export default function InvertPage() {
  ...
  const processImage = () => {
    ...
    img.onload = () => {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          saveAs(blob, `invert_${originalImage.file.name}`);
          toast({ title: "Selesai", description: "Warna gambar telah dibalik." });
        }
    ...
    <div className="container mx-auto p-4 sm:p-8 max-w-6xl space-y-10">
      <div className="flex items-center gap-4">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-black uppercase tracking-tight">Color Inverter</h1>
          <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold">Negative Photo Effect</p>
        </div>
      </div>
            ...
            <Card className="rounded-3xl border-none shadow-xl overflow-hidden">
              <CardHeader className="bg-muted/50 border-b py-4">
                <CardTitle className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                  <IterationCcw className="w-4 h-4 text-accent" /> Actions
                </CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-8">
                <Button className="w-full h-12 bg-accent hover:bg-accent/90 rounded-xl font-bold" onClick={processImage} disabled={isProcessing}>
                  {isProcessing ? <Loader2 className="animate-spin mr-2" /> : <Download className="mr-2" />} Export Negative
                </Button>
              </CardContent>
            </Card>
